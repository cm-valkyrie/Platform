...
class Language
{
    ...
    */
    public function check_lang()
    {
        ...
        if ( Session::exists_var('lang') == false ) Session::set_value('lang', Configuration::$lang_default);
        if ( !file_exists(Security::DS(self::$path_language . Session::get_value('lang') . '.ini')) ) setcookie('lang', Configuration::$lang_default, time() + (86400 * 30), "/");
    }
    ...
    */
    public static function get_lang( $buffer, $section = false, $path = false )
    {
        ...
        if ( $path != false ) $ini = (new self)->format->import_file($path, Session::get_value('lang'), 'ini');
        else $ini = (new self)->format->import_file(self::$path_language, Session::get_value('lang'), 'ini');

        foreach ( $ini[$section] as $key => $value )
        {
            if ( Configuration::$debug_lang === false ) $buffer = str_replace('{$lang.' . $key . '}', $value, $buffer);
